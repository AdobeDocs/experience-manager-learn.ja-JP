---
user-guide-title: AEM と Adobe Target の使用の手引き
user-guide-description: Adobe Experience Manager と Target を使用してパーソナライズされたエクスペリエンスを配信する方法を説明する、エンドツーエンドのチュートリアルです。
product: experience manager
sub-product: Experience Manager Sites
version: 6.5
index: y
jira: KT-2214
source-git-commit: adf3fe30474bcfe5fc1a1e2a8a3d49060067726d
workflow-type: ht
source-wordcount: '64'
ht-degree: 100%

---


# AEM と Target の使用の手引き {#aem-target-tutorial}

+ [概要](./overview.md)
+ [AEM エクスペリエンスフラグメントを使用したパーソナライズ機能](./personalization-use-case-1.md)
+ [Visual Experience Composer を使用したパーソナライズ機能](./personalization-use-case-3.md)
+ [完全な web ページエクスペリエンスのパーソナライズ機能](./personalization-use-case-2.md)
+ 実装 {#aem-target-implementation}
   + [はじめに](./implementation.md)
   + [タグとAdobe I/O を使用した AEM と Adobe Target](./using-launch-adobe-io.md)
   + [レガシーのクラウドサービスを使用する AEM と Adobe Target](./using-aem-cloud-services.md)
