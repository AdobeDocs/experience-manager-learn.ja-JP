...
1. Adobe Asset Link との後続のインタラクション（Adobe Asset Link 拡張機能を使用してアセットなどの参照、検索、チェックイン/チェックアウトを行うと、AEM オーサーに対する HTTP （S） リクエストが行われ、標準のAEM トークン認証ハンドラーを使用して、AEM ログイントークンを使用して検証されます。
