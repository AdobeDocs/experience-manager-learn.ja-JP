...
1. [http://localhost:3000](http://localhost:3000)のSPAを参照します。 すべてがうまく表示されるはずです。

http://localhost:3000](./assets/spa-bootstrap/localhost-3000.png)で実行されている![SPA

## AEM SPA エディターで SPA を開く

[http://localhost:3000](http://localhost:3000)でSPAが実行されている状態で、AEM SPA Editorを使用してSPAを開きましょう。 SPA ではまだ何も編集できません。AEM での SPA の動作を検証するだけです。
...
1. SPA の周りをクリックします
   ...
   http://localhost:3000](./assets/spa-bootstrap/spa-editor.png)で実行されている![SPA
