...
1. [ 最新の Java™ JDK 21 をダウンロードしてインストールする ](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html?fulltext=java*+21*&orderby=%40jcr%3Acontent%2Fjcr%3AlastModified&orderby.sort=desc&layout=list&p.offset=0&p.limit=11)
1. 次のコマンドを実行して、Oracle Java™ 21 JDK がインストールされていることを確認します。
...
>[!CAUTION]
>
>古いバージョンのAEM SDKを実行している場合は、代わりに [Java 11 JDK をダウンロード ](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html?fulltext=java*+11*&orderby=%40jcr%3Acontent%2Fjcr%3AlastModified&orderby.sort=desc&layout=list&p.offset=0&p.limit=11) する必要がある場合があります。 ただし、ベストプラクティスとしては、最新バージョンのAEM SDKを使用することをお勧めします。
...
1. Web ブラウザーで [http://localhost:4502](http://localhost:4502) のローカル AEM オーサーサービスにアクセスします。 初めて起動するときは、インストールが完了するまで数分待つ必要がある場合があります。 通常、ブラウザータブは自動的に開きます。
...
1. 次のコマンドを使用して、AEM プロジェクトまたはサンプル [AEM WKND サイトプロジェクト ](https://github.com/adobe/aem-guides-wknd) をクローンして、ローカル AEM ランタイムにデプロイできます。
...
1. **オーサー** サービスにログインし、[http://localhost:4502/etc/replication/agents.author.html](http://localhost:4502/etc/replication/agents.author.html) に移動します。
...
1. 公開済みコンテンツは、`http://localhost:4503/<your-page-path>.html` で表示できます。 通常、公開済みコンテンツを表示するためにパブリッシュサービスにログインする必要はありません。 ただし、問題が発生したり、ログ、設定などを確認した場合は、[http://localhost:4503/libs/granite/core/content/login.html](http://localhost:4503/libs/granite/core/content/login.html) のパブリッシュサービスにログインできます。
...
>[!WARNING]
>
> クイックスタート Jar を新しいバージョンにアップデートするには、ローカルの開発環境全体を置き換える必要があります。その結果、ローカル AEM リポジトリ内のコード、設定およびコンテンツはすべて失われます。破棄してはいけないコード、設定またはコンテンツを Git に安全にコミットするか、ローカルの AEM インスタンスから AEM パッケージとして書き出すようにします。
...
これは、AEM as a Cloud Serviceには Java™ JDK 21 が必要で、異なるバージョン（おそらく Java™ 11 または 8）を実行しているためです。 この問題を解決するには、[Oracle Java™ JDK 21](https://experience.adobe.com/#/downloads/content/software-distribution/en/general.html?fulltext=Java*+21*&orderby=%40jcr%3Acontent%2Fjcr%3AlastModified&orderby.sort=desc&layout=list&p.offset=0&p.limit=11) をダウンロードしてインストールします。

Oracle Java™ 21 JDK がインストールされたら、コマンドラインからコマンドを実行して、その JDK がアクティブなバージョンであることを確認します。
