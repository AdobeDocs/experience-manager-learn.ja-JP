...
- WKND サイトプロジェクト用の新しいCloud Manager Git リポジトリを作成し、リモートとしてローカル Git リポジトリに追加します。
   ...
   - **リポジトリ名** を入力し（「wknd-site-tutorial」など）、「**保存**」をクリックします。 リポジトリが作成されるのを待ちます。
     ...
     ![&#x200B; リポジトリを追加 &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-repository.png)

   - 右上隅の **リポジトリ情報にアクセス** をクリックし、リポジトリ URL をコピーします。

     ![&#x200B; リポジトリ情報にアクセス &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/access-repo-info.png)
...
- **実稼動以外のパイプラインを追加** ダイアログで、以下を設定します。
   ...
   - **設定** 手順：
     ...
     ![&#x200B; 実稼動以外のパイプライン設定を追加 &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-non-production-pipeline-configuration.png)
   ...
   - **Source コード** 手順：
     ...
     ![&#x200B; 実稼動以外のパイプラインのSource コードを追加 &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/add-non-production-pipeline-source-code.png)

- パイプラインエントリの 3 ドットメニューにある「**実行** をクリックして、新しく作成したコード品質パイプラインを実行します。

  ![&#x200B; コード品質パイプラインの実行 &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/run-code-quality-pipeline.png)
...
![&#x200B; 失敗したパイプラインの実行 &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/failed-pipeline-execution.png)
...
- パイプライン失敗の詳細を、自然言語 **プロンプト** で入力します。 例：
  ...
  ![AEM Development Agent の呼び出し &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/invoke-aem-development-agent.png)
...
- 推論が完了したら、「**フルスクリーンで開く** アイコンをクリックして、詳細なトラブルシューティングプロセスを表示します。
  ...
  ![&#x200B; 全画面表示で開く &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/open-in-full-screen.png)
...
- この場合、エージェントは、問題を修正するために、実装を変更する（`getName()` の方法）か、単体テストを更新する（`getNameTest()` の方法）かを正しく提案しました。 幻覚を避け、開発者に実用的なコード変更を提供しながら、Human-in-the-Loop アプローチを使用しました。
  ...
  ![&#x200B; コードの変更をコピー &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/copy-code-changes.png)
...
![&#x200B; ブランチを比較 &#x200B;](./assets/development-agent-troubleshoot-ci-cd-pipeline/compare-branches.png)
