...
1. 「__保存して閉じる__」を選択します。
1. 更新された __authentication-service__ ユーザー。

   _パッケージを使用する場合は、次の一時的な回避策を使用します。_
   ...
   1. 「__フィルター__」タブを選択し、ルートパス `/home/users/system/cq:services/internal/security/<AUTHENTICATION SERVICE UUID>/keystore` のフィルターを追加します。
      + この `<AUTHENTICATION SERVICE UUID>` は、 __ツール/セキュリティ/ユーザー__、選択 __authentication-service__ ユーザー。 UUID は URL の最後の部分です。
   1. 「__完了__」を選択してから、「__保存__」を選択します。
   1. を選択します。 __ビルド__ ボタン __認証サービスキーストア__ パッケージ。
   1. ビルドが完了したら、「 」を選択します。 __詳細__ > __複製__ 認証サービスのキーストアを AEM パブリッシュにアクティベートします。
