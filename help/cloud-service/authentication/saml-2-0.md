...
### AEM as a Cloud Service プレビューサービスのサポート

AEM プレビューを含むAEM as a Cloud Serviceでは SAML 2.0 がサポートされています。 ただし、AEMの SAML 設定は OSGi 設定に依存しており、AEM プレビューとAEM パブリッシュは同じ OSGi 実行モード解決（`config.publish`）を共有します。 その結果、プレビューと公開用に別々の SAML 設定ファイルを作成することはできません。

代わりに、OSGi 設定内で [&#x200B; 環境固有の設定値 &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-manager-cloud-service/content/implementing/deploying/configuring-osgi#environment-specific-configuration-values) を使用し、プレビュー環境とパブリッシュ環境に [&#x200B; 適切な変数値を設定 &#x200B;](https://experienceleague.adobe.com/ja/docs/experience-manager-cloud-service/content/implementing/deploying/configuring-osgi#cloud-manager-api-format-for-setting-properties) します。
...
高度なユースケースについては、AEMでは、カスタム SAML ログインフック（`com.adobe.granite.auth.saml.SamlLoginHook` インターフェイスを実装する OSGi サービス）の開発をサポートしています。 これらのフックは、SAML 認証プロセス中に実行され、追加のユーザープロビジョニングやカスタムログなどのカスタムロジックを実装するために使用できます。

カスタム SAML ログインフックの開発および登録方法について詳しくは、[&#x200B; カスタム SAML ログインフック &#x200B;](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/implementing/developing/custom-saml-login-hook.html) ドキュメントを参照してください。
