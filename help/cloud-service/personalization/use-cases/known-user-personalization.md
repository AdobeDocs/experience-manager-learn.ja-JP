...
デモ目的で、WKND ユーザーデータが次のシステムにあると仮定します。 各システムは、次の 2 つのカテゴリに分類できるさまざまな種類のデータを格納します。

- **行動データ**:デジタルチャネルでのユーザーインタラクションとアクティビティ(ページビュー、クリック、サイトナビゲーション、ログインステータス、ブラウジングパターン)をキャプチャします
- **トランザクションデータ**:完了したビジネストランザクションを記録し、プロファイル情報(購入、注文履歴、プロファイルの詳細、好み)顧客します

| システム | 目的 | どのようなデータが保存されますか? | データタイプ |
|------|------|------|------|
| AEM | コンテンツ管理システム(CMS)、アドベンチャーのリストと予約、ログイン機能 | ユーザーインタラクション:ページビュー、ログインステータス、サイトナビゲーション ユーザー ID、名前、電子メールなどの最小限のユーザー識別子。 | 行動データ |
...
Adobe Experience Platform(AEP)のさまざまなソリューションを使用して、システム全体でユーザーデータを収集、管理、識別、およびステッチします。 結合された ユーザー データを使用してオーディエンスを作成し、Adobe Target でアクティブ化します。 Adobe Target のアクティビティを使用すると、パーソナライズされたエクスペリエンスが、オーディエンスの条件に一致するユーザーに配信されます。

## Adobe Experience Platform設定

完全な顧客プロファイルを作成するには、行動(ページビューデータ)とトランザクション(WKND Adventureの購入)の両方のデータを収集してストアする必要があります。 行動データはタグプロパティを使用して収集され、トランザクションデータはWKNDアドベンチャー購入システムを使用して収集されます。

その後、トランザクションデータはExperience Platformに取り込まれ、行動データと結合されて完全な顧客プロファイルが作成されます。

この例では、 **スキー** アドベンチャーを購入したユーザーを分類するには、ページビュー データとアドベンチャー購入データが必要です。 データは、システム間で共通の識別子である WKND ユーザー ID （`wkndUserId`）を使用してステッチされます。
...
Adobe Experience Platformに再度アクセスして、行動データが収集され、関連するプロファイルが作成されていることを確認します。 次に、トランザクション データを取り込み、行動データとトランザクション データのステッチを確認し、オーディエンスを作成して構成し、オーディエンスをアクティブにしてAdobe Targetします。

### 行動データとプロファイル作成を検証する

行動データが収集され、関連するプロファイルが作成されることを確認しましょう。

- Adobe Experience Platform で **データセット** をクリックし、 **WKND-RDE-Known-User-Personalization-Behavioral** データセットを開きます。 取り込まれたデータの統計が有効であることを確認します。
...
オーディエンスでは、行動データとトランザクションデータに基づいてユーザーの特定のグループを定義します。 この例では、 **Ski** アドベンチャーを購入し、WKND サイトにログインしているユーザーを認定するオーディエンスが作成されます。

オーディエンスを作成するには、次の手順を実行します。

- Adobe Experience Platform で、左ナビゲーションの「 **Audiences** 」をクリックし、「 **作成 オーディエンス** ボタンをクリックします。 次に、「 **ビルド-ルール** 」オプションを選択し、「 **作成** 」ボタンをクリックします。
  ![作成オーディエンス](../assets/use-cases/known-user-personalization/create-audience.png)

- 「 **作成** 」ステップで、以下を入力します。
   - **名前**:認証されたスキー機器をアップセルする
...
これで完了です。あなたは、システム間でステッチされたデータを使用して完全な顧客プロファイルを構築することにより、既知のユーザーパーソナライズ機能エクスペリエンスを提供する専門家です。
