...
AEM as a Cloud Service **アクションセンター** は、プロジェクトの _非推奨の API_ について通知します。 Cloud Manager パイプラインを使用してコードの最新機能、セキュリティ更新、スムーズなAEM as a Cloud Serviceへのデプロイメントを取得するには、プロジェクトから非推奨（廃止予定）の API を削除します。
