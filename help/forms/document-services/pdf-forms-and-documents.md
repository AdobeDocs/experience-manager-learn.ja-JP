...
この記事では、様々な種類のPDFを作成する方法を説明します。 それぞれの使い方と使い方を理解し、理由を理解するのに役立ちます。 これらのタイプはすべて、プレミアクライアントツールのPDF(Adobe AcrobatDC)の表示と操作に最適です。

AcrobatDCのPDF/Aファイルの例を次に示します。
...
Adobeでは、「PDFフォーム」という用語を使用して、AEM Formsデザイナで作成するインタラクティブでダイナミックなFormsを指します。 Designerで作成するFormsとファイルは、AdobeのXMLFormsアーキテクチャ(XFA)に基づいています。 多くの点で、XFA PDFファイル形式は、従来のPDFファイルよりもHTMLファイルに近い形式になっています。 例えば、次のコードは、XFA PDFファイル内の単純なテキストオブジェクトの見え方を示しています。

![テキストフィールド](assets/text-field.JPG)

XFAFormsはXMLベースです。 このように構造が整い、柔軟な形式を採用しているため、AEM FormsサーバーでDesignerファイルを、従来のPDF、PDF/A、HTMLなど、様々な形式に変換できます。 Designerで、レイアウトエディターの「XMLソース」タブを選択すると、Formsの完全なXML構造を確認できます。 AEM Formsデザイナーで静的XFAFormsと動的XFAの両方を作成できます。

## スタティックPDF

静的XFAPDF formsのレイアウトは実行時に変更されませんが、ユーザーに対してインタラクティブにすることができます。 静的XFAPDF formsの利点のいくつかを次に示します。

* 静的XFAPDF formsのレイアウトは実行時に変更されませんが、ユーザーに対してインタラクティブにすることができます。
* スタティックFormsは、Acrobatの注釈ツールとマークアップツールをサポートしています。
* スタティックFormsを使用すると、Acrobatコメントの読み込みと書き出しが可能です。
* スタティックFormsは、AEM Formsサーバー上で実行できる技術であるフォントサブ設定をサポートします。
* スタティックFormsは、最新のブラウザに付属する組み込みPDFビューアを使用してレンダリングできます。

>[!NOTE]
>
> スタティックPDFは、AEM FormsDesignerでAdobeスタティックPDFフォームとして保存すると作成できます
...
認定ドキュメントは、PDFドキュメントおよびForms受信者に、信頼性と整合性の保証を追加して提供します。
...
* Adobe Acrobatプログラムのフォームツールで上にボルトで留められたインタラクティブフォームフィールド。 これらのフォームツールは、AEM Formsデザイナで利用できる小さなサブセットです。
