---
title: OSGiサービスの作成
description: 署名するフォームを保存するOSGiサービスを作成する
feature: ワークフロー
topics: development
audience: developer
doc-type: tutorial
...
署名が必要なフォームを保存するために、次のコードが記述されています。 署名する各フォームは、一意のGUIDと顧客IDに関連付けられています。 したがって、1つ以上のフォームを同じ顧客IDに関連付けることはできますが、フォームに一意のGUIDが割り当てられます。
...
insert dataメソッドは、データソースによって識別されるデータベースに行を挿入します。 データベースの各行はフォームに対応し、GUIDと顧客IDで一意に識別されます。 フォームデータとフォームURLもこの行に格納されます。 「ステータス」列には、フォームが入力および署名済みかどうかを示します。 値が0の場合、フォームはまだ署名されていません。
...
次のコードは、指定したGUIDに関連付けられたアダプティブフォームデータを取得するために使用されます。 次に、フォームデータを使用してアダプティブフォームに事前入力します。
...
署名式の完了が成功し、フォームに関連付けられたAEMトリガーが完了しました。 ワークフローの最初の手順は、guidと顧客IDで識別される行のデータベースのステータスを更新するプロセス手順です。 また、フォームデータの署名済み要素の値をYに設定し、フォームが入力および署名済みであることを示します。 アダプティブフォームにはこのデータが入力され、xmlデータ内の署名済みデータ要素の値が使用されて適切なメッセージが表示されます。 updateSignatureStatusコードは、カスタムプロセスステップから呼び出されます。
...
次のコードは、ステータスが0の特定のcustomerIDに対する署名の次のフォームを取得するために使用されました。 SQLクエリが行を返さない場合は、文字列&#x200B;**&quot;AllDone&quot;**&#x200B;が返されます。これは、指定した顧客IDに対する署名用のフォームがないことを示します。
...
上記のサービスを含むOSGiバンドルは、[こちら](assets/sign-multiple-forms.jar)からダウンロードできます。