...
# フォームデータを保存するサーブレットの作成

次の手順は、フォームデータを挿入または更新するサーブレットを作成することです。 前の手順で設定したApache Sling接続プールされたDataSourceは、26行目で参照されています。 残りのコードは非常に単純です。 コードは、データベースに新しい行を挿入するか、既存の行を更新します。 保存されたアダプティブフォームデータは、GUIDに関連付けられています。 次に、同じGUIDを使用してフォームデータが更新されます。
