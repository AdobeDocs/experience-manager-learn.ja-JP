---
title: AEM Forms Workflow でコンマ区切りの文字列を文字列の配列に変換しています
description: フォームデータモデルで、入力パラメーターの 1 つとして文字列の配列が含まれている場合は、フォームデータモデルの送信アクションを呼び出す前に、アダプティブフォームの送信アクションで生成されたデータをマッサージする必要があります。
...
# コンマ区切りの文字列を文字列配列に変換しています {#setting-value-of-json-data-element-in-aem-forms-workflow}

入力パラメーターとして文字列の配列を持つフォームデータモデルに基づいてフォームを作成する場合は、送信後のアダプティブフォームデータを操作して、文字列の配列を挿入する必要があります。 例えば、タイプが文字列配列のフォームデータモデル要素にチェックボックスフィールドを連結した場合、チェックボックスフィールドのデータはコンマ区切りの文字列形式になります。 以下に示すサンプルコードは、コンマ区切りの文字列を文字列の配列で置き換える方法を示しています。

## プロセスステップの作成

プロセスステップは、ワークフローで特定のロジックを実行する場合にAEMワークフローで使用されます。 プロセスステップは、ECMA スクリプトまたは OSGi サービスに関連付けることができます。 カスタムプロセスステップで OSGi サービスを実行します。

送信されたデータは次の形式です。 businessUnits 要素の値は、コンマ区切りの文字列で、文字列の配列に変換する必要があります。

![submitted-data](assets/submitted-data-string.png)

フォームデータモデルに関連付けられている REST エンドポイントの入力データは、次のスクリーンショットに示す文字列の配列を受け取ります。 プロセスステップのカスタムコードは、で送信されたデータを正しい形式に変換します。

![fdm-string-array](assets/string-array-fdm.png)

JSON オブジェクトのパスと要素名をプロセスステップに渡します。 プロセスステップのコードでは、要素のコンマ区切り値を文字列の配列に置き換えます。
...
サンプルバンドルは、 [ここからダウンロード](assets/CreateStringArray.CreateStringArray.core-1.0-SNAPSHOT.jar)
