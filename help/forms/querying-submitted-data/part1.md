...
AEM Forms6.3リリースでは、JSONスキーマを基にアダプティブFormsを作成する機能が導入されました。 JSONスキーマを使用したアダプティブFormsの作成について詳しくは、この [記事](https://helpx.adobe.com/jp/experience-manager/6-3/forms/using/adaptive-form-json-schema-form-model.html)。
...
この記事にはMySql 8データベースが使用されています。 MySQLには、 [JSONという新しいデータ型が導入されました](https://dev.mysql.com/doc/refman/8.0/en/json.html)。 これにより、JSONオブジェクトの保存とクエリが容易になります。 送信されたデータは、データベース内のJSON型の列に格納されます。
