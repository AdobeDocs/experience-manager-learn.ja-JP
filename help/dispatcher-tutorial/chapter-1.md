...
これらの情報の一部と部分は、必ず非常に技術的なレベルで開始します。つまり、何をしたいかが既にわかっていると仮定し、どのようにして目的の達成を実現するかについての詳細を提供します。 ディスパッチャーに対して可能 _で不可能なことについて、何が何で何が何が何か、なぜなのかを説明する概念的な論文は一度も見つかりません_ 。
...
プログラミングに関するアドバイスを聞いたこ _とがあるかもしれません。 「まず動かして、それから早くやれ」_」を選択します。全く間違っているわけではない。 ただし、適切なコンテキストがないと、誤って解釈され、正しく適用されない傾向があります。
...
ファイル名は、要求されたリソースのURLから生成されます。 ファイルを要求した場合 `/foo/bar.html` は、/`var/cache/docroot/foo/bar.html`.
...
URLにサフィックスが付い `path/suffix.ext` ている場合、

* `pagename.selectors.html` がフォルダーとして作成される

* `path` フォルダー内のフォル `pagename.selectors.html` ダー

* `suffix.ext` は、 `path` フォルダー内のファイルです。 注意：サフィックスに拡張子が付いていない場合、ファイルはキャッシュされません。
...
1. URLは非常に長くなる可能性があります。 ローカルファイルシステムに「パス」部分を追加す `/docroot` ると、簡単に一部のファイルシステムの制限を超える可能性があります。 Windows上でNTFSでディスパッチャーを実行するのは難しい作業です。 しかし、Linuxを使っても安全です。
...
はリソースへ `/home/path/suffix` のパスなので、URLにサフィックスは含まれません。
...
最初にリクエストする場合は、ファイルとして作成され `home.html` ます。

それ以降の要求では有効な結果が `home.html/suffix.html` 返されますが、ファイルシステム内の位置を「ブロック」するため、2回目のフォルダとして `home.html` 作成することはできず、キャッシュ `home.html``home.html/suffix.html` されません。
...
逆の方法で実行する場合、最初に要求を行い、 `home.html/suffix.html` 次に最初にフォルダーの下に `suffix.html` キャッシュさ `/home.html` れます。 ただし、このフォルダーは削除され、後でリソースとしてリクエストする `home.html` 際にファイルに置き換えら `home.html` れます。
...
最後のチャプターでは、ディスパッチャーがリクエストをキャッシュできない場合に、多数の例外がリストに記載されていました。 しかし、考慮すべき事項が他にもあります。ディスパッチャーはリクエスト _をキャッシュできるので_ 、必ずしもキャッシュする必要があるとは限りま _せん_。

ポイント：通常、キャッシュは容易です。 ディスパッチャーは、応答の結果を格納し、非常に同じ要求が受信された次回にそれを返すだけで済みます。 Right? 違う！

難しい点は、キャッシュの _無効化_ 、 _またはフラッシュ_ （フラッシュ）です。 ディスパッチャーは、リソースが変更された場合に調べ、再度レンダリングする必要があります。
...
削除 `image.png` や、そのパターンに一致する他のすべてのレンディション `image.thumb.png` を行います。
...
*ホームページに* 。 ホームページは、ディスパッチャーによって静的な.htmlファイルとして保存されます。このファイルには、ティーザーやヘッドラインが含まれます。
...
まだ納得してない？ 「ある種の依存関係追跡を導入する方法が *必要だ」と考えるかもしれません*。 まあ、もっと正確に言えば *ある*。 コミュニケ3 AEMの曽祖父には、ページのレンダリングに使用された _セッション_ (session)に依存関係トラッカーが実装されていました。
...
なぜ「.stat」と呼ばれるのか尋ねてもいいでしょうか？ 「.invalidated」とは限りません。 想像してみて下さい、そのファイルをファイルシステムに置くと、Dispatcherは静的にどのリソースを扱う *かを決めるのに役立ちます* 。静的なWebサーバと同様です。 これらのファイルをダイナミックにレンダリングする必要はありません。

しかし、名前の本性は比喩的ではありません。 これはUnixシステムコールから派生し `stat()`、ファイルの変更時刻（他のプロパティの中でも）を返します。
...
2. エージェントがHTTPヘッダーを送信する場合 `CQ-Action-Scope: ResourceOnly`は、つまり、この1つの無効化リクエストで自動無効化はトリガーされません。 このコード( [https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle](https://github.com/cqsupport/webinar-dispatchercache/tree/master/src/refetching-flush-agent/refetch-bundle))の部分は、独自の複製エージェントにとって良い足がかりになる場合があります。

3. `ResourceOnly`に設定すると、自動無効化を防ぐだけです。 必要な依存関係の解決と無効化を実際に行うには、自分で無効化要求をトリガーする必要があります。 パッケージのディスパッチャーフラッシュルール(https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html[](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-flush-rules/index.html))を調べて、実際にどのようにしてそれが起こるかを知ってください。
...
アセットはAEMのDAM領域にアップロードされ、レスポンシブ画像コンポーネントで _参照されるだけです_ 。
...
部品は二つの部分を持つ。最初の部分は画像のHTMLマークアップをレンダリングし、2番目の部分は参照画像のバイナリデータを「スプール」します。 これはレスポンシブデザインを持つ最新のWebサイトなので、単純な `<img src"…">` タグではなく、タグ内の画像のセットを表示し `<picture/>` ます。 デバイスごとに、2つの異なる画像をDAMにアップロードし、画像コンポーネントから参照します。
...
クエリのパラメーターを見たことがあり `?ck=398547283745`ますか。 これらは「キャッシュキラー(&quot;ck&quot;)」と呼ばれます。 考え方としては、クエリパラメーターを追加した場合、リソースはアッチされません。 また、乱数をパラメータの値として追加すると（「398547283745」など）、URLは一意になり、AEMシステムと画面の間の他のキャッシュでもキャッシュできなくなります。 通常、間にある容疑者は、ディスパッチャーの前にある「ワニス」のキャッシュ、CDN、ブラウザーのキャッシュです。 再度：そんな事するな。 リソースをできるだけ長くキャッシュする必要があります。 キャッシュはお客様の友人です。 友達を殺すな。
...
クエリパラメータを持つURLはキャッシュできないので、指紋をクエリパラメータとして使用してはなりません。 指紋にはセレクターまたはサフィックスを使用できます。

ファイルの `/content/dam/flower.jpg``jcr:lastModified` 日付が2018年12月31日23:59であるとします。 指紋付きのURLはで `/content/home/jcr:content/par/respi.fp-2018-31-12-23-59.jpg`す。

このURLは、参照先のリソース(`flower.jpg`)ファイルが変更されない限り安定しています。 したがって、キャッシュは無限にキャッシュされ、キャッシュキラーではありません。

注意：このURLは、レスポンシブ画像コンポーネントで作成および提供する必要があります。 標準搭載のAEM機能ではありません。

これが基本的な考え方です しかし、見落としがちな細かい点がいくつかある。

この例では、コンポーネントが23:59にレンダリングされ、キャッシュされています。 画像が変更されたので、例えば0時になります。  コンポーネント _は_ 、マークアップに新しいフィンガープリントURLを生成します。

君は _.._.と思うかもしれないが、そうはいかない。画像のバイナリだけが変更され、ページを含めるページに手を加えなかったので、HTMLマークアップの再レンダリングは必要ありません。 したがって、ディスパッチャーはページに古い指紋を提供し、その結果イメージの古いバージョンを提供します。
...
遅延管理者は通常、「自動無効化をjpgに&#x200B;_設定し、statfileレベルをゼロに設定します。これは、常にあらゆる種類のキャッシュの問題に役立ちます_」。 そのアドバイスはテクニカルフォーラムで見つかり、無効化問題の解決に役立ちます。

今までのところ、statfileレベルについては話し合っていない。 基本的に、自動無効化は同じサブツリー内のファイルに対してのみ機能します。 ただし、ページとアセットは通常、同じサブツリーに存在しないという問題があります。 ページは下の場所にあり、アセット `/content/mysite` は下の場所にあり `/content/dam`ます。
...
このようにして、カスタムディスパッチャーフラッシュエージェントは、変更が発生した場合に/content/site-aに対して簡単にリクエストおよび無効化のリクエストを送信でき `/content/dam/site-a`ます。
...
は、無効化されたリソースのどれにも関連しません。 一時保存? 「ダミー」リソースを無効にしただけで、「ホーム」が無効であると考えるための自動無効化に依存していました。 画像自体が _物理的に_ 削除されない場合があります。 このため、キャッシュは増大し、拡大します。 イメージを変更してアクティブ化すると、ディスパッチャーのファイルシステムに新しいファイル名が付けられます。
...
[Versioned Clientlibs](https://adobe-consulting-services.github.io/acs-aem-commons/features/versioned-clientlibs/index.html) は、この方法を使用するモジュールです。

しかしここでは、URLの指紋を持つ別の注意事項に直面することができます。URLをコンテンツに結び付けます。 URLを変更しないと（変更日を更新するなど）、コンテンツを変更できません。 それが第一に指紋のデザインです。 ただし、新しいCSSとJSファイルを使用し、新しい指紋を持つ新しいURLをリリースするとします。 すべてのHTMLページには、従来の指紋付きURLへの参照が引き続き含まれます。 したがって、新しいリリースを一貫して動作させるには、すべてのHTMLページを一度に無効にして、新しく指紋したファイルを参照して再レンダリングを強制する必要があります。 同じライブラリに依存する複数のサイトがある場合は、相当量の再レンダリングが行われる可能性があります。この場合、を利用することはできません `statfiles`。 ロールアウト後にパブリッシュシステムのロードピークを確認する準備をしてください。 青緑色のデプロイメントでは、キャッシュウォーミングを使用します。または、ディスパッチャーの前にあるTTLベースのキャッシュを使用します。可能性は無限です。
...
特定のコンポーネントに対して特別な処理を行う必要がある場合は、そのパスとセレクターに専用のサーブレットを登録し、コンポーネントの代わりに変換を行います。 我々はここで「.respi」を例に挙げ selector. グローバルURLスペース(など `/content/dam`)で使用されるセレクター名を追跡し、名前の競合を回避するために適切な命名規則を持つようにすることをお勧めします。
...
欠けているのは品質の価値です。 コンポーネントは、作成者がどのプロパティを入力したかを把握しています。マークアップをレンダリングする際に、クエリパラメータとしてイメージレンダリングサーブレットに簡単に渡すことができます。例 `flower.respi2.jpg?quality=60`:
...
これは再びキャッシュをバイパスし、パブリッシュシステムで読み込みを作成します。 悪い考えかもしれません この問題を軽減するには、小さなパラメーターのサブセットのみをフィルタリングします。 のみを許可する場合 `q-20, q-40, q-60, q-80, q-100`。
...
AEMがブラウザーに伝えている。 「知らない `q-41`。 でもね…『』について聞いてみ `q-40` て。

これにより、会話にリクエスト応答ループが追加されます。これは少しオーバーヘッドですが、フル処理を行うよりも安価で `q-41`す。 また、既にキャッシュされているファイルを利用でき `q-40`ます。 ただし、302個の応答はディスパッチャーにキャッシュされません。AEMで実行されるロジックについて説明します。 何度も繰り返す。 だから、スリムで速くした方がいい。
...
このパターンを _逆スプーラと呼びます_。 逆スプーラは、すべての良好なキャッシュ無効化プロパティを持つために、画像リソースに基づく必要があります。
...
私たちは _statfileについて前に簡単に述べた_ 。 これは、自動無効化に関連します。

自動無効化が設定されているディスパッチャーファイルシステム内のキャッシュファイルは、最終変更日が `statfile's` 最終変更日より古い場合は無効と見なされます。

>[!NOTE]
>
>最後に変更された日付とは、キャッシュされたファイルがクライアントのブラウザから要求され、最終的にファイルシステムに作成された日付です。 リソースの `jcr:lastModified` 日付ではありません。

statfile(`.stat`)の最終変更日は、AEMからの無効化要求がディスパッチャーで受信された日付です。
...
自動無効化と前述のstatfileを導入したとき、変更があると *きはすべて無効と見なされ* 、すべてのファイルは相互依存関係にあります。

それはあまり正確ではない。 通常、共通のメインナビゲーションルートを共有するすべてのファイルは相互に依存します。 ただし、1つのAEMインスタンスで複数のWebサイト( *独立した* Webサイト)をホストできます。 共通のナビゲーションを共有しない（実際は、何も共有しない）。

サイトAに変更があるので、サイトBを無効にするのは無駄ではないでしょうか。 はい。 そうでなくてもいいのです

ディスパッチャーは、サイトを相互に分離する簡単な方法を提供します。を参照し `statfiles-level`てください。

これは、ファイルシステムのどのレベルから定義される数で、2つのサブツリーが「独立」と見なされます。

statfilelevelが0のデフォルトのケースを見てみましょう。

![/statfileslevel &quot;0&quot;:_ _.stat_ _はdocrootに作成されます。 無効化ドメインは、すべてのサイトを含むインストール全体にわたって](assets/chapter-1/statfile-level-0.png)

`/statfileslevel "0":` ファイルがdocrootに作成され `.stat` ます。 無効化ドメインは、すべてのサイトを含むインストール全体にわたって適用されます。

どのファイルが無効になっても、ディスパッチャーdocrootの最上部にある `.stat` ファイルは常に更新されます。 したがって、無効にする `/content/site-b/home`と、のすべてのファイルも無効になります。これは、docroot内の `/content/site-a``.stat` ファイルよりも古くなったためです。 無効にした場合、必要なものは明らかではありません `site-b`。

この例では、をに設定 `statfileslevel` し `1`ます。

現在は、を発行し、その結果無効になる `/content/site-b/home` か、以下の他のリソースを発行すると、に `/content/site-b`ファイルが作成され `.stat``/content/site-b/`ます。

以下のコンテンツ `/content/site-a/` は影響を受けません。 このコンテンツは、の `.stat` ファイルと比較され `/content/site-a/`ます。 我々は2つの個別の無効化ドメインを作成しました。
...
大規模なインストールは、通常、もう少し複雑で深い構造になっています。 一般的なスキームは、ブランド、国、言語別にサイトを構築することです。 この場合、statfilelevelをさらに高く設定できます。 _1_ は、ブランドごとに無効化ドメインを作成し、国ごとに _2_ 、言語ごとに _3_ を作成します。
...
前者は `statfileslevel` 2 _、_&#x200B;後者は _3_&#x200B;が必要です。

理想的な状況ではありません。 これを _3に設定すると_、サブブランチととの間の小さなサイトでは自動無効化が機能しません `/home``/products``/about`。

この値を _2に設定すると_ 、大きなサイトでは、宣言し、依存関係にあり、そうでない可能性があ `/canada/en``/canada/fr` ることを意味します。 したがって、の各無効化 `/en` も無効になり `/fr`ます。 これにより、キャッシュのヒット率がわずかに低下しますが、キャッシュされた古いコンテンツを配信するよりも効果的です。
...
どちらが正しいレベルですか？ これは、サイト間の依存関係の数に依存します。 ページのレンダリングで解決する選択は、「ハード依存関係」と見なされます。 このガイドの最初にTeaser _コンポーネントを導入した際に_ 、このような __ 包含を示しました。

_ハイパーリンク_ （ハイパーリンク）は、より柔軟な依存関係です。 1つのウェブサイト内のハイパーリンクが作成される可能性が非常に高く、ウェブサイト間のリンクがあるとは考えにくい。 単純なハイパーリンクは、通常、Webサイト間に依存関係を作成しません。 サイトからfacebookに設定した外部リンクを考えてみてください。Facebook上で何か変化が起きた場合やその逆の場合は、ページをレンダリングする必要はありません。
...
透明にして下位レベルにならないようにする、同じコンテンツを持つページが複数あり、システムを「ゲーム」しないようにする、各ページのヘッダーセクションで [タグを各関連ページに設定する](https://support.google.com/webmasters/answer/189077?hl=en)`<link rel="alternate">` :
...
このスキームは、多数のリンクだけでなく、いくつかの問題も生み出しました。 n _言語の_ pに必要なリンク数は、 _p x_ (n _2<sup></sup>_-n)です。各ページは、それ自体(_-n_)を除き、互いにリンクし合っています(_n x n_)。 このスキームは各ページに適用されます。 20ページの4言語の小さなサイトがある場合、それぞれ240の _リンクになります_ 。
...
当社のプロジェクトでは、新しい相対ページが頻繁に表示されていました。 しかし、「代替」リンクとしては具現化されなかった。 例えば、ドイツのウェブサイトに掲載された `de-de/produkte` ページは、他のサイトにはすぐには表示されませんでした。
...
それは、 `statfiles` レベルを1に設定することを意味していたので、無効化ドメインは大きすぎました。
...
よりリーナブルな設定では、無効化要求はコンテンツを受け取った _後_ 、発行システムから送信されます。 「発行インスタンスからのディスパッチャーキャッシュの[無効化](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#InvalidatingDispatcherCachefromaPublishingInstance)」で詳細を説明しています。
...
通常は、リソースパスとサフィックスで派生できる情報を使用して、見つからないヘッダーをApacheサーバー内のリソースに再適用 `mod_headers` し始めました。 しかしそれでは必ずしも十分ではなかった

特に迷惑な点は、ディスパッチャーでも、ブラウザーに対する最初の __ キャッシュされない応答が、全範囲のヘッダーを持つ発行システムから来ているのに対し、それ以降の応答は、限られたヘッダーのセットを持つディスパッチャーによって生成されたことです。
...
Cache-ControlとPragmaは、公式のHTTPヘッダーで、CDNなどの上位のキャッシュ層に伝播され、それらのヘッダーが解釈されます。 この `Dispatcher` ヘッダーは、ディスパッチャーがキャッシュしないためのヒントです。 この変数は、上位のキャッシュレイヤーがキャッシュを許可しながら、ディスパッチャーにキャッシュを禁止するように指示する場合に使用できます。 実は、それが役に立つかもしれないケースを見つけるのは難しい。 でも、どこかには確かにある。
...
通常は、Apacheのを使用して静的に行います。また、より個別のコントロールが必要な場合は、AEMのCache-ControlとExpires Headerを格納します。 `mod_expires`

ブラウザー内のキャッシュされたドキュメントは、最新の3つのレベルを持つことができます。

1. _保証された新規_ — ブラウザーはキャッシュされたドキュメントを使用できます。
...
ブラウザーのキャッシュ内のドキュメントが、新しいリクエスト時に配信されるドキュメントと同じかどうかを判断するために、ブラウザーはの `Last-Modified` 日付を使用できます。 ブラウザーがサーバーに問い合わせます。

「6 _月10日のバージョンがありますが、更新が必要ですか。_」と入力し、

&quot;_304 - Your version is still up ude to date_&quot;(リソースを再転送しない限り、サーバーは

HTTPヘッダーに「_200 - here is a mreter recent version_」、HTTP本文に実際にある最新のコンテンツ。

2つ目のパートを機能させるには、更新を要求する参照ポイントがあるように、 `Last-Modified` 日付をブラウザに送信します。

先ほど説明したように、 `Last-Modified` 日付がディスパッチャーによって生成される場合、キャッシュされたファイル（および日付）はブラウザーがファイルを要求したときに生成されるので、リクエストによって異なる場合があります。 代わりに、「eタグ」を使用します。日付の代わりに、実際のコンテンツを識別する数値（ハッシュコードの生成など）です。

[ACS Commonsパッケージの「](https://adobe-consulting-services.github.io/acs-aem-commons/features/etag/index.html)Etag Support __ 」では、このアプローチが使用されます。 ただし、これには次の価格が付いています。Eタグはヘッダーとして送信する必要がありますが、ハッシュコードの計算では応答を完全に読み取る必要があるので、応答を配信する前にメインメモリに完全にバッファリングする必要があります。 Webサイトにキャッシュされていないリソースが存在する可能性が高く、もちろんAEMシステムで消費されるメモリを監視する必要がある場合は、遅延に悪影響を与える可能性があります。

URL指紋を使用している場合は、有効期限を非常に長く設定できます。 指紋付きのリソースは、ブラウザーでいつでもキャッシュできます。 新しいバージョンには新しいURLが付けられ、古いバージョンは更新する必要はありません。

スプーラーパターンを導入した際にURL指紋を使用しました。 (CSS、JS)からの静的ファイル `/etc/design` はほとんど変更されず、指紋として使用するのに適した候補となります。
...
AEM UIを構成するGraniteライブラリは、かなり長い時間キャッシュできます。 また、サイトの静的ファイル（フォント、CSSおよびJavaScript）をブラウザーでキャッシュすることもできます。 内の画像 `/content/dam` でも、ページ上のテキストをコピーするほど頻繁に変更されないので、通常は約15分間キャッシュできます。 画像は、AEMでインタラクティブに編集されません。 AEMにアップロードされる前に、まず編集および承認が行われます。 したがって、テキストほど頻繁に変更されていないと想定できます。
...
AEMだけに頼るな。 このパスを使用すると、キャッシュのルートディレクトリ `/home.html` と同じパスになります。 フィニッシュやドイツ、カナダのウェブサイトの「ホーム」か？ また、ディスパッチャーにファイルがある場合、無効化要求が発生したときに無効にする必要があることを、ディスパッチャーはどのようにして知る `/home.html``/content/brand/fi/fi/home` のでしょう。

ドメインごとに異なるドクルートを持つプロジェクトを見てきました。 デバッグとメンテナンスは悪夢でした。実際、完全に動くのは見たことがありません。

キャッシュを再構築することで問題を解決できます。 すべてのドメインに対して1つのdocrootがあり、サーバー上のすべてのファイルが最初に起動した場合に、無効化要求を1:1で処理でき `/content`ます。

切り捨ても簡単でした。  に従った設定が原因で、AEMで切り捨てられたリンクが生成され `/etc/map`ました。

リクエストがディスパッチャー `/home.html` をヒットする場合、最初に発生するのは、パスを内部的に拡張する書き換えルールを適用することです。
...
ファイルシステムにはプレーン `/content`ベースのパスがあり、AuthorとPublishにもあります。これは多くのデバッグに役立ちます。 正しい無効化については言及しないでください。問題はなくなりました。
...
はい — でもこれはAEMの部分だけです。 注意：ディスパッチャーは、キャッシュ `404 – not found` や `500 – internal server error` 応答を行いません。
...
これはもちろんお客様が見たものではありません。 ディスパッチャーでは、次のように登録し `ErrorDocuments` ました。
...
1つ目は、常に同じページ `error-404.html` であることです。 したがって、「「Your search for &quot;_producten_&quot; did not beild a result」のような個々のメッセージはありません。 私たちはそれと一緒に住むのが楽でした。

2つ目は、内部サーバエラーが発生した場合、あるいはAEMシステムが停止した場合でも、AEMにエラーページを表示するように依頼する方法はありません。 ディレクティブで定義された後に必要な要求も失敗し `ErrorDocument` ます。 この問題を回避するために、定義された場所から定期的にエラーページを取り出し、ディレクティブで定義された静的ファイルの場所に格納するcronジョブ `wget``ErrorDocuments` を実行しました。
...
>[!NOTE]
>
>このパターンには _、_ Gateway _が必要です。このGatewayは_ 、各要求を _傍受し_ 、実際の認証（ディスパッチャーへの要求の許可または拒否）を実行します。 SSOシステムが _認証子の場合_、Option 3を実装する必要があるユーザーのIDのみを確立します。 SSOシステムのハンドブックの「SAML」や「OAuth」などの用語を読むと、これはOption 3を実装する必要のある強いインジケーターです。
...
この「キャッシュの無効化ストーム」と呼ばれる問題を軽減するために、 `statfile` 解釈に対してあまり厳密でない場合があります。

ディスパッチャーは、自動無効化のためにを使用するよ `grace period` うに設定できます。 これは、内部的に、 `statfiles` 変更日に若干時間を追加します。

例えば、今日12:00 `statfile` の変更時刻があり、2分に設定さ `gracePeriod` れているとします。 次に、自動無効化されたすべてのファイルは、12:01および12:02で有効と見なされます。 12:02の後に再レンダリングされます。

参照設定では、正当な理由で2分 `gracePeriod` のデータが提示されます。 「2分？ それはほとんど何もない。 10分待てばコンテンツが現れる…」  そのため、少なくとも10分後にコンテンツが表示されると仮定して、10分のように長い期間を設定する必要が生じる可能性があります。

>[!WARNING]
>
>これは、そのように機能してい `gracePeriod` るわけではありません。 猶予期間は _、ドキュメントの無効化が保証された後の時間では_ なく、無効化が行われない時間枠になります。 このフレーム内に含まれる後続の無効化のたびに _、時間枠が長くなります_ 。これは無限に長くなる可能性があります。

例を使って実際にど `gracePeriod` のように作業を行うかを説明します。
...
その他…コンテンツは無効にされません。 猶予期間 *内の各無効化は* 、事実上猶予期間を延長する。 無効嵐を天気にかけ `gracePeriod` るために…だが、最終的に雨の中に出て行く必要がある…だから、シェルターに隠れるのを永久に防ぐために、かなり `gracePeriod` 短くしておけ。

#### 決定論的猶予期間

無効化嵐を乗り切る方法を紹介します。 それは単なる考えだ。 実稼働環境では試していませんがこのアイデアを皆さんに伝えるのに十分興味深いコンセプトを見つけました

通常のレプリケーション間隔 `gracePeriod` がお客様の時間より短い場合、予測できない長さになる可能性があり `gracePeriod`ます。
...
「カスタム無効化スクリプト」（参照を参照）を使用します。このスクリプトは、無効化が発生した後に実行されます。 このスクリプトは、 `statfile's` 最終変更日を読み取り、次の間隔の停止に切り上げます。 Unixシェル・コマンド `touch --time`で、時間を指定します。
...
[helpx.adobe.com — ディスパッチャーの設定](https://helpx.adobe.com/jp/experience-manager/dispatcher/using/dispatcher-configuration.html)
...
自動再フェッチを使用すると、ある程度緩和できます。 無効になったページのほとんどは、自動無効化後もディスパッチャーに物理的に保存されます。 それらは古いものと _見なされる_ 。 _自動再取得_ ：古いコンテンツを再取得するために、公開システムへの __ 1つの要求を開始しながら、古いページを数秒間提供することを意味します。
...
ディスパッチャーは、メンテナンスを目的とした要求から発行システムを遮断することで、セキュリティを強化します。 例えば、自分やURLを公開し `/crx/de` ないようにし `/system/console` ます。
...
ディスパッチャーをフィルターとして使用する別の方法は、 `dispatcher.any`
...
バージョンBでは、一重引用符 `'` を使用して _正規式パターンをマークします_。 「任意の文字」は、を使用して表し `.*`ます。

_これに対し_、グロビングパターンは重複の引用符を使用し `"` 、のような単純なプレースホルダーのみを使用でき `*`ます。

その違いを知っていれば、簡単なことです。でも、知らないと、簡単に見積もりを組み合わせて、晴れた午後の設定をデバッグできます。 今は警告を受けている。

「構成 `'/url'` では分かるでもフィルターの中 `'/glob'` に何が？

このディレクティブは、メソッドとパスを含む要求文字列全体を表します。 それは～のために立つ

`"GET /content/foo/bar.html HTTP/1.1"`

これは、パターンの比較対象となる文字列です。 初心者は、最初の部分 `method` (GET、POSTなど)を忘れがちです。 パターン
...
各定義の前にあるような「行番号」 `/005` についての最後の一語。
...
>[!WARNING]
>
>より高度な設定では、フィルタリングルールを多数のファイルに分割し、メインの `dispatcher.any` 設定ファイルに含めます。 ただし、新しいファイルでは新しい名前空間が導入されません。 したがって、あるファイルに「001」というルールがあり、別のファイルに「001」というルールがある場合は、エラーが発生します。 意味的に強い名前を付ける理由が増えました。
...
ディスパッチャーのドキュメントに、以下のすべてのディレクティブが説明されています。 [https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html](https://helpx.adobe.com/jp/experience-manager/dispatcher/using/dispatcher-configuration.html)
...
プレゼンテーション「The undervelud power of content invalidation」, 「adaptTo()」会議(ポツダム2018 [https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html)](https://adapt.to/2018/en/schedule/the-underappreciated-power-of-content-invalidation.html)
