...
プログラミングに関するアドバイスを聞いたかもしれません _「最初に動作させ、次に速く作ります。」_ を使用して作成します。全く間違っているわけではありません ただし、正しいコンテキストがないと、誤って解釈され、正しく適用されない傾向があります。
...
これらはAEMで絶対に有効です。 ローカル開発マシン（Dispatcher なし）に問題は発生しません。 また、UAT または負荷テストで問題が発生しない可能性が高くなります。 我々が直面している問題は、ほとんどのテストを抜け出すほど微妙です。  ピーク時に、対処する時間が限られている場合、サーバーへのアクセス権がない場合や、修正するリソースがない場合は、これはハードヒットします。 私たちはそこに行った…
...
今のところ、これがパターンかアンチパターンかを決めるのは、あなた次第です。 上で説明した問題を軽減する方法について、既に良いアイデアを心に留めているかもしれません。 良かった。 その後、他のプロジェクトがどのようにこれらの問題を解決したかを見たいと思うはずです。
...
それでも、1 つの基本的な注意事項が解決されるわけではありません。画像が異なるページで複数回使用された場合、それらのページの下にキャッシュされます。 キャッシュのシナジーはあまりありません。
...
例えば、 `statfile` の変更時刻は今日 12:00 で、 `gracePeriod` が 2 分に設定されている場合。 その後、自動無効化されたすべてのファイルは、12:01 および 12:02 に有効と見なされます。 12:02 の後に再レンダリングされます。
