...
# スタイルシステムのコードの作成方法について{#understanding-how-to-code-for-the-aem-style-system}
...
>[!NOTE]
>
>AEM Style Systemは、 [AEM 6.3 SP1](https://helpx.adobe.com/jp/experience-manager/6-1/release-notes/sp3-release-notes.html) + [Feature Pack 20593で導入されました](https://www.adobeaemcloud.com/content/marketplace/marketplaceProxy.html?packagePath=/content/companies/public/adobe/packages/cq630/featurepack/cq-6.3.0-featurepack-20593)。
>
>このビデオでは、We.Retail Titleコンポーネントが [コアコンポーネントv2.0.0以降を継承するように更新されていることを前提としてい](https://github.com/Adobe-Marketing-Cloud/aem-core-wcm-components/releases)ます。
...
提供されるAEMパッケージ(**technical-review.sites.style-system-1.0.0.zip**)は、サンプルのタイトルスタイル、Web.Retail Layoutコンテナおよびタイトルコンポーネント用のサンプルポリシー、およびサンプルページをインストールします。
...
CSSを好むユーザーの場合、このコードスニペットの下に、このコンパイル先のCSSが表示され [!DNL LESS] ます。
...
上記 [!DNL LESS] は、次のCSSにExperience Managerしてネイティブにコンパイルされています。
...
**良好** — コンポーネント内のすべての要素は、BEM表記でアドレス指定可能です。
...
**不良** -リスト要素とリスト要素は、要素名でアドレス指定できるだけです。
...
>[!NOTE]
>
>Style Systemは、 [BEM](https://en.bem.info/)`BLOCK``BLOCK--MODIFIER` BEMで指定されている同じ要素に対して、およびが適用されない点で、BEMとの技術的な相違を小さく示します [](https://en.bem.info/)。
>
>代わりに、製品の制約により、 `BLOCK--MODIFIER` が `BLOCK` 要素の親に適用されます。
>
>他の [BEMのテナントはすべて、](https://en.bem.info/) BEM.

* CSSの定義を明確にし、再利用を可能にするには、 [LESS](https://lesscss.org/) (AEMでネイティブにサポート)や [SCSS](https://sass-lang.com/) （カスタムビルドシステムが必要）などのプリプロセッサーを使用します。

* セレクターの重み付け/特異性を一定に保つ。これにより、識別しにくいCSSカスケードの競合を回避し、解決できます。
* 各スタイルを個別のファイルに編成します。
   * これらのファイルは、LESS/SCSSを使用して組み合わせるこ `@imports` とも、生のCSSが必要な場合にも、HTMLクライアントライブラリファイルを含めることも、カスタムのフロントエンドアセット構築システムを使用して組み合わせることもできます。
* 複雑なスタイルを多く混在させない。
...
* レスポンシブグリッドに付属しているので、 `BLOCK--MODIFIER` 直接のスタイル設定は避けてください。 この要素の表示を変更すると、レスポンシブグリッドのレンダリングと機能に影響する可能性があるので、レスポンシブグリッドの動作を変更する意図がある場合は、このレベルのスタイルのみに影響します。
* を使用してスタイル範囲を適用 `BLOCK--MODIFIER`します。 はコンポーネントで使用 `BLOCK__ELEMENT--MODIFIERS` できますが、はコンポーネントを `BLOCK` 表し、コンポーネントはスタイルが設定されているので、スタイルは「定義済み」でスコープが使用され `BLOCK--MODIFIER`ます。
...
ネストされたコンポーネントの場合、これらのネストされたコンポーネント要素のCSSセレクターの深さは、第3レベルのセレクターを超えます。 親コンポーネントのスコープで指定されているネストされたコンポーネントに対して同じパターンを繰り返し `BLOCK`ます。 つまり、ネストされたコンポーネントを3番目のレベルに開始し、ネストされたコンポーネント `BLOCK` を4番目のセレクターレベル `ELEMENT` に移動します。
...
* 可能な場合は、style-Javascriptを再使用します。
   * 例えば、1つのコンポーネントの複数のスタイルで、そのコンポーネントの画像を背景の画像に移動する必要がある場合、そのスタイルJavaScriptを1回実装し、複数のスタイルに添付することができ `BLOCK--MODIFIERs`ます。
...
* ちらつきや不要な再描画を避けるには、を使用してコンポーネントを非表示にし、JavaScriptでのすべてのDOM操作が完了したら `BLOCK--MODIFIER BLOCK`それを表示します。
