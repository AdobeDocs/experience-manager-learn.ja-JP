...
## Analysis Workspace, Analytics — レポートスイートのセットアップ
...
ただし、XDM スキーマには、eVarやイベントデータを送信するAdobe Analytics固有のフィールドグループはありません。 プラットフォームにeVarやイベントデータが保存されないように、既存のスキーマを更新する代わりに新しい XDM スキーマが作成されます。
